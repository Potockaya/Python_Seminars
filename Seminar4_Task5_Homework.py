...
# koef = [randint(0, 101) for i in range(k)]+[randint(1, 101)]
# poly1 = '+'.join([f'{(j,"")[j==1]}x^{i}' for i,
#                   j in enumerate(koef) if j][::-1])
# # Поиск и замены:
# poly1 = poly1.replace('x^1+', 'x+')
# poly1 = poly1.replace('x^0', '')
# poly1 += ('', '1')[poly1[-1] == '+']
# poly1 = (poly1, poly1[:-2])[poly1[-2:] == '^1']
# print(poly1)

# koef = [randint(0, 101) for a in range(k)]+[randint(1, 101)]
# poly2 = '+'.join([f'{(b,"")[b==1]}x^{a}' for a,
#                   b in enumerate(koef) if b][::-1])
# # Поиск и замены:
# poly2 = poly2.replace('x^1+', 'x+')
# poly2 = poly2.replace('x^0', '')
# poly2 += ('', '1')[poly2[-1] == '+']
# poly2 = (poly2, poly2[:-2])[poly2[-2:] == '^1']
# print(poly2)


# my_file = open("Task5-1.txt", "w")
# my_file.write(poly1)
# my_file.close()

# my_file = open("Task5-2.txt", "w")
# my_file.write(poly2)
# my_file.close()
